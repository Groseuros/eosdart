...
class Api {
  ...
  Future<CachedAbi?> getCachedAbi(String? accountName,
    ...
    try {
      ...
      throw "fetching abi for $accountName: $e";
    }
    if (cachedAbi == null) {
  ...
  Future<List<BinaryAbi>> getTransactionAbis(dynamic transaction,
      {bool reload = false}) async {
    var accounts = (transaction["actions"] as List)
        .map((action) => action["account"].toString())
        .toList();
    ...
    var t = uniqueAccounts.map((f) async {
      return BinaryAbi(f, (await getCachedAbi(f, reload: reload))!.rawAbi);
    });
  ...
  Future<ser.Contract?> getContract(String? accountName,
      {bool reload = false}) async {
    if (!reload && contracts[accountName] != null) {
      return contracts[accountName];
    }
    var abi = await (getAbi(accountName, reload: reload) as FutureOr<Abi>);
  ...
  Future<List<ser.SerializedAction>> serializeActions(
      List<ser.Action> actions) async {
    var ac = actions.map((act) async {
      var contract = await (getContract(act.account) as FutureOr<ser.Contract>);
      var sr = ser.serializeAction(
  ...
  Future<dynamic> deserializeTransactionWithActions(Object transaction) async {
    ...
    var deserializedTransaction = deserializeTransaction(transaction as Uint8List);
    var deserializedActions =
        await deserializeActions(deserializedTransaction.actions);
    return {
      "deserializedTransaction": deserializedTransaction,
  ...
  Future<dynamic> transact(dynamic transaction,
    ...
    if (blocksBehind is int && expireSeconds != null) {
      // use config fields to generate TAPOS if they exist
      if (info == null) {
        info = await rpc!.getInfo();
      }
      var refBlock = await rpc!.getBlock(info.headBlockNum! - blocksBehind);
    ...
    var act = await serializeActions((transaction["actions"] as List)
    ...
    var pushTransactionArgs = PushTransactionArgs(
        signatures: [], serializedTransaction: serializedTransaction);

    if (sign) {
      var availableKeys = await signatureProvider!.getAvailableKeys();
      var requiredKeys = await rpc!
          .getRequiredKeys(AuthorityProviderArgs(transaction, availableKeys));
  ...
  Future<dynamic> transactTest(
      Map testInfo, String chainId, dynamic transaction,
      {bool broadcast = true,
      bool sign = true,
      int? blocksBehind,
      int expireSeconds = 180}) async {
...
//     }
  ...
  Future<dynamic> pushSignedTransaction(PushTransactionArgs push) async {
    return rpc!.pushTransaction(push);
  }

  // eventually break out into TransactionValidator class
  bool hasRequiredTaposFields(dynamic tr) {
    return tr["expiration"] != null &&
        tr["ref_block_num"] != null &&
        tr["ref_block_prefix"] != null;
  }
} // Api
